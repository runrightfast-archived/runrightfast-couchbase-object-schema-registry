...
(function() {
	...
	ObjectSchemaRegistryDatabase.prototype.createObjectSchema = function(objectSchema) {
		...
		return when.promise(function(resolve, reject) {
			if (!lodash.isObject(objectSchema)) {
				reject(new Error('objectSchema is required'));
				if (log.isDebugEnabled()) {
					log.debug('createObjectSchema(): objectSchema is not an object\n' + JSON.stringify(objectSchema, undefined, 2));
				}
				return;
			}

			if (log.isDebugEnabled()) {
				log.debug('createObjectSchema():\n' + JSON.stringify(objectSchema, undefined, 2));
			}
	...
	ObjectSchemaRegistryDatabase.prototype.getObjectSchemas = function(ids) {
		...
		return when.promise(function(resolve, reject) {
			...
			self.cb.getMulti(ids, null, function(error, result) {
				if (log.isDebugEnabled()) {
					log.debug('getObjectSchemas(): getMulti():\n' + JSON.stringify({
						error : error,
						result : result
					}, undefined, 2));
				}
				if (error) {
					...
					if (errors.length > 0) {
						log.error('getObjectSchemas() : ' + error);
						reject({
							error : error,
	...
	ObjectSchemaRegistryDatabase.prototype.getObjectSchemaVersions = function(namespace) {
		...
		return when.promise(function(resolve, reject) {
			...
			var q = self.cb.view('namespace_version', 'namespace_version', {
				stale : false,
				startkey : [ namespace ],
				endkey : [ namespace + '/' ],
				inclusive_end : true,
	...
	/**
	 * NOTE: this api can also be used to page through all ObjectSchemas in the
		...
		return when.promise(function(resolve, reject) {
			...
			if (replace) {
				lodash.forEach(couchbaseViews, function(view) {
					deleteDesignDocsPromises.push(when.promise(function(resolve, reject) {
